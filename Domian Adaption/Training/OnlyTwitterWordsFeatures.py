...
tweets_path = 'F:\\IIT Delhi\\Semester 4\\Natural Language Processing\\Assignment\\Part 2\\hilary.csv'
# Labels
...
with open(tweets_path, 'r') as csvfile:
    ...
    for tweet in tweets:
        ...
        ###########################################
        while(True):
            match = re.search(r'([A-Za-z][A-Za-z0-9\']+)', tt)
            if match :                
                start = match.start()
                end = match.end()
                word = tt[start:end]
...
fo = open("TwitterWordsOfHillary", "w")
for x in features_with_freq:
    fo.write(x[0] + '\n')
fo.close()